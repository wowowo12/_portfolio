var Slide = (function(){
	...
	function Slide(_wrap, _option) {
		...
		if(_option) {
			this.option = _option; // nullChk가 undefined, null 일때 true
			...
			if(this.nullChk(this.option.pagerDefClass)) this.option.pagerDefClass = "w3-transparent";
			if(this.nullChk(this.option.pagerActClass)) this.option.pagerActClass = "w3-transparent";
		}
		else {
			//_option을 생략했을때 기본값
			this.option = {
	...
	Slide.prototype.initPingpong = function(obj) {
		...
		*/
		obj.interval = setInterval(ani, obj.option.delay, obj); //obj를 받아서
		function ani(obj) {
	...
	Slide.prototype.pagerInit = function(obj) {
		...
		//name으로 pager생성 (nullChk이니까 undefined이거나 null일때)
...
var Slide = (function () {
    ...
    Slide.prototype.slidePingpong = function () {
        ...
        $(this.slides).delay(this.option.delay).animate({
            "left": -(this.wid * this.now) + "px"
        }, this.option.speed, function () {
...
*/
//clone() 하나를 clone하여 새로운 메모리 공간으로 집어넣는다.
//참조타입의 변수를 단순히 append하면, 순서만 바뀔 뿐 복사되지 않는다.